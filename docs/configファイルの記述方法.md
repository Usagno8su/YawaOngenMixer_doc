...
階層構造は半角スペースで表現します。
このファイルでは「半角スペース2つ」でひとつ下の階層となります。
...
各項目の詳細についてはconfファイルにコメントを書いているため、そちらを確認して下さい。
...
- conp_tatie  
  ...
  セリフと合わせて何か特殊な表示を行いたい場合は、この機能が使えると思います。
...
- tatie_h_p  
  立ち絵を画面の高さの何％にするか指定します。  
  100％以上の設定も可能です。

- tatie_position_wps　tatie_position_hps  
  「tatie_muki」の位置を基準として、そこからさらに指定した値分（％）立ち絵を移動させます。
  
  - 「tatie_position_wps」は右へ移動させます。（マイナスで左です。）  
  - 「tatie_position_hps」は上へ移動させます。（マイナスで下です。）
  
  これで、「立ち絵の上半身のみ表示させる」といった使い方が可能となります。

   > ※補足（ImageMagickの仕様を知っている方へ）  
　 　「-geometry」オプションは起点から見て中心の方がプラスとなりますが、  
　 　分かりづらいため、どこを起点としても右と上がプラスになるように調整しています。
　 

- fps  
  入力される立ち絵と、出力される動画のフレームレート指定です。
  
  現在の仕様上は「1」にしても問題ありませんが、将来的に目や口を動かす場合には調整する必要があります。

- enatext  
  字幕を表示するか指定（yesを入れると表示）します。  
...
これで、「四国めたん」の場合は「立ち絵」と「フォントカラー」と「フォントのフチの色」が変更されます。  
それ以外の項目は「defo」の内容が適用されます。
...
これで、「四国めたん」の「ささやき」スタイルの場合は「フォントサイズ」が「8」に変更され、「フォントカラー」は「#F08080」に変更されます。  
これ以外の「立ち絵」と「フォントのフチの色」は「kyara」の設定が適用されますが、「フォントカラー」はこの「kyara_st」の設定が優先されます。

上記２つにない設定項目は「defo」の内容が適用されます。
...
もしこの番号がついたファイルの「スタイルを含めたキャラ名」が「四国めたん（ささやき）」であっても、こちらの設定が優先されます。

それ以外の「フォントカラー」は「kyara_st」の設定が適用され、  
「立ち絵」と「フォントのフチの色」は「kyara」の設定が適用されます。

そして残りの設定項目は「defo」の内容が適用されます。
