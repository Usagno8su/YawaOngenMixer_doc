...
Ruby のビルド等に必要なパッケージを apt 経由でインストールします。
```
Host$ lxc exec rb01 -- sh -c "apt install build-essential libssl-dev libreadline-dev zlib1g-dev libpq-dev unzip"
```
...
本体のダウンロードと解凍
```
ubuntu@rb01$ wget https://github.com/Usagno8su/YawaOngenMixer/releases/download/v0.1.0/YawaOngenMixer-v0.1.0.zip
ubuntu@rb01$ unzip YawaOngenMixer-v0.1.0.zip
```
